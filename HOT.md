...
- Validate the new tab-aware refresh card against already-open Instagram, YouTube, TikTok, unsupported sites, and browser-owned tabs so popup feedback matches real Chromium behavior
...
- The tab-aware refresh action depends on popup-side `chrome.tabs.query` and `chrome.tabs.reload` behavior, so it still needs browser-side validation on supported tabs, unsupported sites, and browser-owned pages
